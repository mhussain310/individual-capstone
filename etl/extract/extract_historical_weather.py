...
previous_date = current_date - timedelta(
    days=30
)  # 1-month prior to current date (API Restriction)

current_date_str = current_date.strftime("%Y-%m-%d")
previous_date_str = previous_date.strftime("%Y-%m-%d")

url = f"{BASE_URL}/history.json?key={weather_api_key}&q=New_York&dt={previous_date_str}&end_dt={current_date_str}"
weather_data = get_url(url).json()
